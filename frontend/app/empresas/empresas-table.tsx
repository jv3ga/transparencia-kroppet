  ...
  return (
    <div className="space-y-4">
      ...
      <div className="md:hidden flex flex-col gap-2">
        ...
        {rows.map((e, i) => (
          <div key={e.id} className="rounded-xl border border-border bg-card p-4 flex items-center gap-3">
            <span className="text-xs text-muted-foreground tabnum w-6 text-right shrink-0"
                  style={{ fontFamily: "var(--font-mono)" }}>
              {i + 1}
            </span>
            <div className="flex-1 min-w-0">
              <Link href={`/contratos?empresa_id=${e.id}`}
            ...
            <div className="text-right shrink-0">
              ...
              <p className="text-[10px] text-muted-foreground tabnum"
                 style={{ fontFamily: "var(--font-mono)" }}>
                {e.num_contratos.toLocaleString("es-ES")} contratos
              </p>
            </div>
          </div>
        ))}
